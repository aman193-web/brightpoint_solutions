...
import {
  ...
  FIXTURE_ASSEMBLIES, DEVICE_ASSEMBLIES, GENERIC_ASSEMBLIES, ALL_ASSEMBLIES, categoryOf,
  STATUS_CFG, LIB_TYPE_CFG, UNIT_OPTIONS,
} from './libraryData';
import { ColumnLibraryView } from './ColumnLibraryView';
import { AssemblyBuilderView } from './AssemblyBuilderView';
import { LiveBOM } from './LiveBOM';
import { recordSaved, markJustSaved } from '../../lib/recentAssemblies';
 ...
 */
/**
 * Libraries has two primary modes.
 *
 * Browse is the existing Column browser experience; Build hands the whole
 * working area to the guided configurator. The Workbench workspace and its
 * `Workbench | Column browser` switcher were removed on 9 August 2026 — with
 * one browse workspace left, a switcher offering a single option was noise, so
 * `Browse | Build` took its slot.
 *
 * `WorkbenchLibraryView.tsx` is retained on disk but no longer imported or
 * reachable, the same treatment the guided Builder got.
 */
type LibraryPrimaryMode = 'browse' | 'build';

export function LibraryView() {
  const [mode, setMode] = useState<LibraryPrimaryMode>('browse');
  ...
  /** The only switcher: Browse | Build. */
  const modeSwitcher = (
    <div style={{ display: 'flex', border: '1px solid #E5E7EB', borderRadius: 7, overflow: 'hidden', flexShrink: 0 }}>
      {(['browse', 'build'] as LibraryPrimaryMode[]).map((m) => (
        <button
          key={m}
          onClick={() => setMode(m)}
          title={m === 'browse'
            ? 'Find an assembly that already exists'
            : 'Configure a new assembly — full screen, guided'}
          style={{
  ...
  /*
   * One switcher now, in the slot the workspace switcher used to hold: last in
   * the toolbar, right-aligned. Both toolbars share the same padding and the
   * control has the same width in either mode, so right-aligning it lands it on
   * the same pixel in Browse and Build.
   */

  /**
   * A save lands the assembly in Recently Created, drops Build Mode and flags
   * the row to flash — so the estimator sees where it went without hunting.
   */
  function handleSaved(a: Assembly, cat: CategoryCode, libraryId: string) {
    const lib = libraries.find((l) => l.id === libraryId);
    recordSaved({ assembly: a, cat, libraryId, libraryName: lib?.name ?? 'Company Library' });
    markJustSaved(a.id);
    setMode('browse');
    toast.success('Assembly saved', {
      description: `${a.name} — filed under ${CATEGORIES.find((c) => c.code === cat)?.name ?? cat}.`,
    });
  }

  /** Names already filed, so Save can warn about a clash. */
  const existingNames = ALL_ASSEMBLIES.map((a) => ({ name: a.name, cat: categoryOf(a) }));
